...
class DockerManager extends BaseServerlessManager {
    ...
    async shutdown() {
        ...
        logger.info('DockerManager shutting down...');
    ...
    async stopAllContainers() {
        ...
        logger.info(`Stopping ${this.pool.length} containers...`);
        
        const terminatePromises = this.pool.map(containerInfo => 
            this.terminateResource(containerInfo).catch(err => {
                logger.error(`Error stopping container ${containerInfo.name}:`, err.message);
            })
        );
        ...
        try {
            ...
            logger.error('Error during container termination:', err);
        }
