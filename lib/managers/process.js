...
class ProcessManager extends BaseServerlessManager {
    ...
    formatResourceInfo(resourceInfo) {
        ...
        info.alive = resourceInfo.process && !resourceInfo.process.killed;
        return info;
    }

    async getResourceUsage(resourceInfo) {
        if (!resourceInfo.process || resourceInfo.process.killed) {
            return null;
        }

        try {
            const stats = await pidusage(resourceInfo.process.pid);
            return {
                cpu: stats.cpu, // Percentage
                memory: stats.memory // Bytes
            };
        } catch (err) {
            // pidusage might fail if process is gone or permissions
            return null;
        }
    }

    async getOrCreateProcessInPool() {
        if (this.isShuttingDown) {
            throw new Error('ProcessManager is shutting down');
        }

        const scriptPath = this.scriptDirPath && this.scriptFiles
            ? path.join(this.scriptDirPath, this.scriptFiles[0])
            : null;

        if (!scriptPath) {
            throw new Error('Script path is not configured. Provide scriptDirPath and scriptFiles in constructor.');
        }
    ...
    async createResource(config = {}) {
        const scriptPath = config.scriptPath || (this.scriptDirPath && this.scriptFiles
            ? path.join(this.scriptDirPath, this.scriptFiles[0])
            : null);
        ...
        try {
            const port = await getAvailablePort();
            const processName = `process-${port}-${Date.now()}`;
            return await this.createProcess(scriptPath, port, processName);
        } catch (err) {
    ...
    async onShutdown() {
        if (this.pool.length === 0) {
            // Clear pidusage history to prevent memory leaks/open handles
            pidusage.clear();
        }
    }
}
