...
class BaseServerlessManager {
    constructor(options = {}) {
        ...
        this.preWarmConfig = options.preWarmConfig || {};

        // Script configuration
        this.scriptDirPath = options.scriptDirPath || null;
        this.scriptFiles = options.scriptFiles || ['index.js'];
    ...
    async poolWatcher() {
        ...
        this.watcherInterval = setInterval(async () => {
            ...
            if (this.pool.length > 0 && now - this.lastRequestTime > this.poolCheckInterval) {
                // Don't evict if we are at or below minPoolSize
    ...
    async startPoolWatcher() {
        ...
        // Initial pre-warm check
        if (this.pool.length < this.minPoolSize) {
            this.preWarm().catch(err => {
                logger.error(`Error during initial pre-warming: ${err.message}`);
            });
        }
    }
    ...
    async healthCheck() {
        ...
        // Trigger pre-warm if we dropped below minPoolSize
        if (this.pool.length < this.minPoolSize && !this.isShuttingDown) {
            this.preWarm().catch(err => logger.error(`Error pre-warming after health check: ${err.message}`));
        }
    ...
    /**
     * Pre-warm resources to meet minPoolSize
     */
    async preWarm() {
        if (this.isShuttingDown) return;

        const needed = this.minPoolSize - this.pool.length;
        if (needed <= 0) return;

        logger.info(`Pre-warming ${needed} ${this.getResourceType()}s...`);

        const promises = [];
        for (let i = 0; i < needed; i++) {
            if (this.pool.length + promises.length >= this.maxPoolSize) break;

            promises.push(
                this.createResource(this.preWarmConfig)
                    .then(resource => {
                        if (resource && this.canCreateNewResource()) {
                            this.addToPool(resource);
                            logger.info(`Pre-warmed ${this.getResourceType()} ${resource.name}`);
                        } else if (resource) {
                            // Pool full, terminate
                            this.terminateResource(resource).catch(() => { });
                        }
                    })
                    .catch(err => {
                        logger.error(`Failed to pre-warm ${this.getResourceType()}: ${err.message}`);
                    })
            );
        }
     ...
     * @param {Object} config - Configuration for creating the resource
     * @returns {Promise<Object>} The created resource info
     */
    async createResource(config) {
        throw new Error('createResource() must be implemented by subclass for pre-warming');
    }
}
