...
class K8sManager extends BaseServerlessManager {
            ...
            if (this.pool.length > 0 && now - this.lastRequestTime > this.poolCheckInterval) {
                ...
                if (podToRemove) {
                    try {
                        ...
                        console.log(`Stopped and removed pod: ${podToRemove.name} (port ${podToRemove.port})`);
                    } catch (err) {
                        console.error(`Error stopping pod ${podToRemove.name}:`, err.message);
                    }
                }
            }
    ...
    async terminateResource(podInfo) {
        ...
        try {
            ...
            console.log(`Stopped and removed pod: ${podName}`);
        } catch (err) {
    ...
    async onShutdown() {
        ...
        for (const [podName, portForwardProcess] of this.portForwardProcesses.entries()) {
            if (portForwardProcess && !portForwardProcess.killed) {
                portForwardProcess.kill('SIGTERM');
            }
        }
    ...
    async getOrCreatePodInPool(scriptDirPath, scriptFiles = ['index.js']) {
        ...
        if (selectedPod) {
            ...
            if (isAlive) {
                ...
                console.warn(`Pod ${selectedPod.name} is not running, removing from pool`);
